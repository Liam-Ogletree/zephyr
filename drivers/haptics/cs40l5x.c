...
#define CS40L5X_CONFIG_IF_LOGGING(dev)                                                             \
	COND_CODE_1(IS_ENABLED(CONFIG_LOG),	                                                   \
...
#define CS40L5X_T_DEFAULT_DELAY_MS    K_MSEC(1)
#define CS40L5X_T_RLPW_MS             K_MSEC(1)
#define CS40L5X_T_IRS_MS              K_MSEC(3)
#define CS40L5X_T_DSP_READY           K_MSEC(10)
#define CS40L5X_T_WAKESOURCE_MS       K_MSEC(10)
...
static const struct cs40l5x_multi_write cs40l5x_b0_internal_boost[] = {
	{.buf = (uint32_t[]){0x00002018, 0x00003321, 0x04000010}, .len = 3},
};

static const struct cs40l5x_multi_write cs40l5x_b0_external_boost[] = {
	{.buf = (uint32_t[]){0x00002018, 0x00003201}, .len = 2},
	{.buf = (uint32_t[]){0x00004404, 0x01000000}, .len = 2},
};

static const struct cs40l5x_multi_write cs40l5x_b0_errata[] = {
	{.buf = (uint32_t[]){0x00000040, 0x00000055}, .len = 2},
	{.buf = (uint32_t[]){0x00000040, 0x000000AA}, .len = 2},
	{.buf = (uint32_t[]){0x00003014, 0x08012E16}, .len = 2},
	{.buf = (uint32_t[]){0x00003808, 0xC0000004}, .len = 2},
	{.buf = (uint32_t[]){0x0000380C, 0xC8710230}, .len = 2},
	{.buf = (uint32_t[]){0x0000388C, 0x04E0FFFF}, .len = 2},
	{.buf = (uint32_t[]){0x0000649C, 0x01818461}, .len = 2},
	{.buf = (uint32_t[]){0x00000040, 0x00000000}, .len = 2},
	{.buf = (uint32_t[]){0x02BC21B8, 0x00000302, 0x00000001, 0x00018B41, 0x00009920},
	 .len = 5}};

static const struct cs40l5x_multi_write cs40l5x_b0_errata_external_boost[] = {
	{.buf = (uint32_t[]){0x00000040, 0x00000055}, .len = 2},
	{.buf = (uint32_t[]){0x00000040, 0x000000AA}, .len = 2},
	{.buf = (uint32_t[]){0x00005C00, 0x00000400}, .len = 2},
	{.buf = (uint32_t[]){0x00004220, 0x8000007D}, .len = 2},
	{.buf = (uint32_t[]){0x00004200, 0x00000008}, .len = 2},
	{.buf = (uint32_t[]){0x00004240, 0x510002B5}, .len = 2},
	{.buf = (uint32_t[]){0x00006024, 0x00522303}, .len = 2},
	{.buf = (uint32_t[]){0x00000040, 0x00000000}, .len = 2},
	{.buf = (uint32_t[]){0x02804348, 0x00040020, 0x00183201, 0x00050044, 0x00040100, 0x00FD0001,
			     0x0004005C, 0x00000400, 0x00000000, 0x00422080, 0x0000007D, 0x00040042,
			     0x00000008, 0x00050042, 0x00405100, 0x00040060, 0x00242303,
			     0x00FFFFFF},
	 .len = 18}};

static const struct cs40l5x_multi_write cs40l5x_irq_clear[] = {
	{.buf = (uint32_t[]){CS40L5X_REG_IRQ1_INT1, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
			     0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
			     0xFFFFFFFF},
	 .len = 11},
	{.buf = (uint32_t[]){CS40L5X_REG_IRQ1_INT14, 0xFFFFFFFF}, .len = 2},
	{.buf = (uint32_t[]){CS40L5X_REG_IRQ1_INT18, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
			     0xFFFFFFFF},
	 .len = 6},
};

static const struct cs40l5x_multi_write cs40l5x_irq_masks[] = {
	{.buf = (uint32_t[]){CS40L5X_REG_IRQ1_MASK1, 0x03FFFFFF, 0xFFDF7FFF}, .len = 3},
	{.buf = (uint32_t[]){CS40L5X_REG_IRQ1_MASK4, 0xE0FFFFFF}, .len = 2},
	{.buf = (uint32_t[]){CS40L5X_REG_IRQ1_MASK8, 0x7C000FFF, 0x0101C033, 0x0000F00C}, .len = 4},
	{.buf = (uint32_t[]){CS40L5X_REG_IRQ1_MASK20, 0x15FFF000}, .len = 2},
};
...
static const struct cs40l5x_multi_write cs40l5x_pseq_internal[] = {
	{.buf = (uint32_t[]){CS40L5X_REG_WSEQ_POWER, 0x00000000, 0x00201800, 0x00003321, 0x00030404,
			     0x00000010, CS40L5X_WSEQ_TERMINATOR},
	 .len = 7},
};
			       ...
			       const uint32_t val, const k_timeout_t timeout)
{
	...
	do {
		...
		(void)k_sleep(CS40L5X_T_DEFAULT_DELAY_MS);
...
{
	const k_timepoint_t end = sys_timepoint_calc(CS40L5X_T_WAKESOURCE_MS);
	CS40L5X_CONFIG_IF_LOGGING(dev);
	int error;
				    ...
				    uint32_t pins)
{
	...
	struct cs40l5x_config *const config = (struct cs40l5x_config *const)data->config;
	struct cs40l5x_trigger_gpios *const gpios = &config->trigger_gpios;
	struct cs40l5x_trigger_item item;
	int error, warning;
	uint8_t i;
...
{
	struct cs40l5x_config *const config = (struct cs40l5x_config *const)dev->config;
	struct cs40l5x_trigger_gpios *const gpios = &config->trigger_gpios;
	struct cs40l5x_data *const data = dev->data;
...
static int cs40l5x_process_interrupts(const struct device *const dev,
				      const uint32_t *const irq_intX)
{
	CS40L5X_CONFIG_IF_LOGGING(dev);
	int error = 0;

	if (FIELD_GET(CS40L5X_MASK_IRQ1_AMP, irq_intX[CS40L5X_INT1]) != 0) {
		LOG_INST_ERR(config->log, "amplifier short failure");
		error = -EIO;
	}

	if (FIELD_GET(CS40L5X_MASK_IRQ1_TEMP, irq_intX[CS40L5X_INT8]) != 0) {
		LOG_INST_ERR(config->log, "overtemperature failure");
		error = -EIO;
	}

	if (FIELD_GET(CS40L5X_MASK_IRQ1_BST, irq_intX[CS40L5X_INT9]) != 0) {
		LOG_INST_ERR(config->log, "boost failure");
		error = -EIO;
	}

	if (FIELD_GET(CS40L5X_MASK_IRQ1_VDDB, irq_intX[CS40L5X_INT10]) != 0) {
...
static int cs40l5x_retrieve_interrupt_statuses(const struct device *const dev,
					       uint32_t *const irq_intX)
{
	uint32_t irq_maskX[CS40L5X_NUM_IRQ1_INT];
	int error;

	error = cs40l5x_regmap_burst_read(dev, CS40L5X_REG_IRQ1_INT1, irq_intX, 10);
	if (error < 0) {
		return error;
	}

	error = cs40l5x_regmap_read(dev, CS40L5X_REG_IRQ1_INT14, &irq_intX[CS40L5X_INT14]);
	if (error < 0) {
		return error;
	}

	error = cs40l5x_regmap_burst_read(dev, CS40L5X_REG_IRQ1_INT18, &irq_intX[CS40L5X_INT18], 5);
	if (error < 0) {
		return error;
	}

	error = cs40l5x_regmap_burst_read(dev, CS40L5X_REG_IRQ1_MASK1, irq_maskX, 10);
	if (error < 0) {
		return error;
	}

	error = cs40l5x_regmap_read(dev, CS40L5X_REG_IRQ1_MASK14, &irq_maskX[CS40L5X_INT14]);
	if (error < 0) {
		return error;
	}

	error = cs40l5x_regmap_burst_read(dev, CS40L5X_REG_IRQ1_MASK18, &irq_maskX[CS40L5X_INT18],
					  5);
	if (error < 0) {
		return error;
	}
...
static void cs40l5x_interrupt_worker(struct k_work *work)
{
	...
	uint32_t irq1_status, irq_intX[CS40L5X_NUM_IRQ1_INT];
	int error;
	...
	error = cs40l5x_retrieve_interrupt_statuses(data->dev, irq_intX);
	if (error < 0) {
		LOG_INST_DBG(config->log, "failed to read IRQ registers (%d)", error);
		goto exit_pm;
	}

	error = cs40l5x_process_interrupts(data->dev, irq_intX);
	if (error < 0) {
		goto exit_pm;
	}

	if (irq_intX[CS40L5X_INT2] & CS40L5X_MASK_IRQ1_V2MBOX1) {
				      ...
				      uint32_t pins)
{
	...
	CS40L5X_CONFIG_IF_LOGGING(data->dev);
	int warning;
...
static int cs40l5x_boost_configuration(const struct device *const dev)
{
	...
	switch (data->rev_id) {
	case CS40L5X_REVID_B0:
		if (!CS40L5X_ANY_DEV_USE_EXTERNAL_BOOST) {
			return cs40l5x_regmap_multi_write(dev, cs40l5x_b0_internal_boost,
							  ARRAY_SIZE(cs40l5x_b0_internal_boost));
		} else if (!CS40L5X_ANY_DEV_USE_INTERNAL_BOOST || config->external_boost) {
			return cs40l5x_regmap_multi_write(dev, cs40l5x_b0_external_boost,
...
static int cs40l5x_reset(const struct device *const dev)
{
	...
	(void)k_sleep(CS40L5X_T_RLPW_MS);
...
{
	struct cs40l5x_config *const config = (struct cs40l5x_config *const)dev->config;
	struct cs40l5x_trigger_gpios *const trigger_gpios = &config->trigger_gpios;
	struct cs40l5x_data *const data = dev->data;
	int error = 0;
...
int cs40l5x_calibrate(const struct device *const dev)
{
	...
	error = cs40l5x_calibrate_f0(dev, &f0);
	if (error < 0) {
		goto error_mutex;
	}
...
int cs40l5x_logger(const struct device *const dev, enum cs40l5x_logger logger_state)
{
	...
	} else if (logger_state != CS40L5X_LOGGER_NO_CHANGE) {
...
static int cs40l5x_start_output(const struct device *const dev)
{
	...
	if (error < 0) {
		...
		item.bank = cs40l5x_bank_from_cmd(data->output);
...
static int cs40l5x_init(const struct device *dev)
{
	...
	if (IS_ENABLED(CONFIG_HAPTICS_CS40L5X_CALIBRATION)) {
		if (k_sem_init(&data->calibration_semaphore, 0, CS40L5X_SEMAPHORE_MAX) < 0) {
			return -ENOMEM;
		}
	}
	...
	if (CS40L5X_ANY_DEV_USE_FLASH_STORAGE && config->flash != NULL) {
		if (!device_is_ready(config->flash)) {
			LOG_INST_WRN(config->log, "flash device is not ready (%s)",
				     config->flash->name);
		}
	}
...
#if CONFIG_PM_DEVICE
static int cs40l5x_deinit(const struct device *dev)
{
