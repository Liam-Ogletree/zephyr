...
static int flash_le25s20xa_allow_program(const struct device *dev)
{
	...
	ret = flash_le25s20xa_block_protect(dev, false);
	if (ret < 0) {
		return ret;
	}

	/* Require additional write enable for subsequent program or erase. */
	return flash_le25s20xa_write_enable(dev, true);
}
...
static int flash_le25s20xa_write_helper(const struct device *dev, off_t offset, const void *buf,
					size_t len)
{
	const struct flash_le25s20xa_config *config = dev->config;
	uint8_t cmd[4] = {
	...
	return flash_le25s20xa_prevent_program(dev);
}
				 ...
				 size_t len)
{
	...
	if (len > config->parameters.write_block_size) {
		LOG_DBG("invalid write length, require 0 < len <= %u",
			config->parameters.write_block_size);
		return -EINVAL;
	}

	if (!IN_RANGE(offset, 0, config->size - len)) {
		LOG_DBG("invalid parameters, require 0 < offset + len <= %llu", config->size);
		return -EINVAL;
	}
	...
	ret = flash_le25s20xa_write_helper(dev, offset, buf, len);
	if (ret < 0) {
		LOG_DBG("failed to program %s", dev->name);
	}
...
static int flash_le25s20xa_erase_helper(const struct device *dev, off_t offset, size_t size,
					const uint32_t sector_size)
{
	const struct flash_le25s20xa_config *config = dev->config;
	int ret;
